    ...
    def _build_simplified_feature_store_request(self, aggregation_element):
        ...
        if use_parallel:
            for attr_name, d in pexpressions.items():
                encoded_list = kv_array.encode_list(d["values"][d["first_index"] : d["last_index"] + 1])
                print(f"encoded_list={encoded_list}")
                print(f"encoded_list type is {type(encoded_list).__name__}")
                encoded_array = encoded_list.decode()
